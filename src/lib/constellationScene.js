...
export function createConstellationScene(canvas, options = {}) {
  ...
  const MR = 140, MR_SQ = MR * MR
  const MAX_CONN = 350
  const BASE_CONN_ALPHA = 0.30
  ...
  for (let i = 0; i < particleCount; i++) {
    particles.push({
      ...
      baseAlpha: (0.35 + Math.random()*0.45) * alphaMultiplier,
      size: (2.5 + Math.random()*3.5) * Math.min(window.devicePixelRatio, 2),
  ...
  function animate(time) {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const breathe = Math.sin(el*0.5 + p.phase) * 0.18
      let boost = 0
      if (phase === 'gather') boost = eased * 0.25
      if (phase === 'hold') boost = 0.30
      if (phase === 'scatter') boost = (1-eased) * 0.25
      ...
      if (burstState.active) {
        ...
        if (burstElapsed > burstState.duration) {
          ...
          burstBoost = (1 - bp) * 0.7 * Math.sin(bp * Math.PI)
        }
      }
    ...
    if (phase === 'gather') { acd += eased*80; acdSq = acd*acd; acap += eased*0.18 }
    else if (phase === 'hold') { acd += 80; acdSq = acd*acd; acap += 0.18 }
    else if (phase === 'scatter') { acd += (1-eased)*80; acdSq = acd*acd; acap += (1-eased)*0.18 }

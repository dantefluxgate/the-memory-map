...
export default function SharedClosing({ summary, particleRef }) {
  ...
  useEffect(() => {
    if (isVisible) {
      ...
      particleRef?.current?.setColor(0xe0b080)
  ...
  return (
    ...
    >
      {/* Sparkle twinkles for the finale */}
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(circle, rgba(224,176,128,0.16) 0%, rgba(224,176,128,0.04) 40%, transparent 65%)',
          opacity: phase >= 1 ? 1 : 0,
      ...
      <div className="text-center max-w-[550px] relative z-10">
        ...
        {summary?.dominant_themes && (
          ...
          >
            {summary.dominant_themes.map((theme, i) => (
              <span
                key={theme}
                className="font-body text-[9px] uppercase tracking-[0.1em] text-accent-primary/70 border border-accent-primary/25 rounded-full px-4 py-1 animate-stagger-up"
        ...
        >
          <p className="font-body text-[10px] uppercase tracking-[0.2em] text-text-tertiary/50 flex items-center justify-center gap-2">
            Made with love on
            <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor" className="text-accent-primary/60 heart-glow">
              <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
            </svg>
            The Memory Map
          </p>
          <button
            onClick={() => navigate('/')}
            className="group font-body text-sm text-accent-primary hover:text-accent-secondary transition-all duration-500 cursor-pointer border border-accent-primary/30 rounded-full px-8 py-2.5 hover:border-accent-primary/50 relative overflow-hidden glow-button"
          >
            {/* Hover glow effect */}
            <span className="absolute inset-0 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"
              style={{ boxShadow: '0 0 30px rgba(224,176,128,0.20), inset 0 0 24px rgba(224,176,128,0.08)' }}
            />
